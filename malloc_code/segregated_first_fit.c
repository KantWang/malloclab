...
void seg_init(void) {
    ...
    // free 블록 크기 2^0부터 2^32까지 NULL값으로 초기화
    // 1, 2, 3-4, 5-8, ..., 2049-4096, 4097-무한
    for (int i = 0; i < 32; i++)
        PUT_P(seg_listp + (WSIZE * i), NULL);
}
...
static void* seg_find(int size){
    ...
    int i = 32;
    for (; i > 0; i--)
        if ((size & (1 << (i))) > 0) // size와 2**i 간 일치하는 비트가 있으면
            break;

    // 크기가 일치하는 집합의 주소를 리턴
    seg = seg_listp + (WSIZE * i);
    return seg;
}

int mm_init(void)
{
    /* Create the initial empty segregated heap */
    seg_init();
    ...
    heap_listp += (2 * WSIZE); // head_listp는 inplicit때와 동일하게 prologue header의 주소 + 1을 가리킨다
...
static void *find_fit(size_t adjust_size){ 
    ...
    int i = 32;
    for (; i > 0; i--)
        if ((adjust_size & (1 << (i))) > 0) // &연산 결과 0이 아닌 시점이 adjust_size > 크기인 시점
            break;
    
    // 해당 집합 안 원소들을 탐색해서 딱 맞는 크기를 찾는다
    int j = i;
    for (; j <= 32; j++) { // 2**j 크기의 free 블록이 없는 경우 j++
        seg = seg_listp + (WSIZE * j);
        if (GET_P(seg) != (char*)NULL) { // seg에 prev free 블록의 주소가 저장
            best = (1 << (j + 1)); // 
            for (bp = PREV_FREE_BLKP(seg); bp != (char*)NULL; bp = PREV_FREE_BLKP(bp)) { // 해당 사이즈 구간에 해당하는 free 블록들 탐색, 연결리스트의 first는 최신 순으로 업데이트 되기 때문에 뭐가 가장 남는 메모리를 적게할지는 끝까지 가봐야 안다
                if (adjust_size <= GET_SIZE(HDRP(bp)) && GET_SIZE(HDRP(bp)) - adjust_size < best) {
                    best_bp = bp;
                    best = GET_SIZE(HDRP(bp)) - adjust_size; // 남는 사이즈
                }
            }
            // 해당 집합의 free 블록 모두 탐색을 마치고
            if (best_bp != (char*)NULL)
                return best_bp; // bp 찾았다!
        }
    }
...
void *mm_malloc(size_t size)
{
    ...
    if (size <= DSIZE)
        adjust_size = DSIZE * 2; 
    // 할당 요청의 용량이 2words 초과 시, 적절한 8byte의 배수의 용량 할당
    else 
...
static void push_first(void* bp) {
    ...
    PUT_P(PREVRP(bp), PREV_FREE_BLKP(seg)); // 새로운 free 블록의 prev에 직전 first free 블록의 prev 주소 저장
    PUT_P(NEXTRP(bp), seg); // 새로운 free 블록의 next에 prologue prev 주소 저장
    PUT_P(PREVRP(seg), bp); // prologue prev에 bp 저장
}
